...
export default function Hopdong() {
  ...
  const fetchHosoList = useCallback(async () => {
    ...
    try {
      setHosoLoading(true);
      const response = await fetch('http://127.0.0.1:8080/api/hoso');
      if (response.ok) {
  ...
  const columns = [
    ...
    },
    {
      ...
      width: 100,
      render: (val) => val
        ? <a href={/^https?:\/\//i.test(val) ? val : `http://127.0.0.1:8080${val}`} target="_blank" rel="noreferrer" className="btn btn-secondary btn-xs" style={{ display: 'inline-flex', alignItems: 'center', gap: '4px', padding: '4px 8px' }}>
