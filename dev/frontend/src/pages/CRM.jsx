...
export default function CRM() {
  ...
  const fetchData = async () => {
    try {
      const [leadsRes, statsRes] = await Promise.all([
        fetch('http://127.0.0.1:8080/api/crm/leads'),
        fetch('http://127.0.0.1:8080/api/crm/stats')
      ]);
  ...
  const submitStatusChange = async (leadId, newStatus, extraData = {}) => {
    try {
      await fetch(`http://127.0.0.1:8080/api/crm/leads/${leadId}/status`, {
        method: 'PUT',
  ...
  const handleCreateLead = async (e) => {
    e.preventDefault();
    try {
      const res = await fetch('http://127.0.0.1:8080/api/crm/leads', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
