...
function NodeActionBar({ task, onChanged }) {
  ...
  if (task.status === 'in_progress') {
    ...
    const submitButton = <button type="button" className="btn btn-primary btn-sm"
      disabled={busy || unapprovedChecklistItems.length > 0} onClick={submitForAcceptance}>
      <Send size={14} /> Nộp nghiệm thu
    </button>
    if (unapprovedChecklistItems.length === 0) return submitButton
    return <div className="employee-workspace-task-modal__gate">
      {submitButton}
      <small>
        {pendingChecklistItems.length > 0
          ? `Tích xong checklist bên dưới rồi mới nộp nghiệm thu được — còn ${pendingChecklistItems.length} việc`
