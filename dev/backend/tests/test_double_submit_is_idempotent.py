...
class DoubleSubmitTests(unittest.TestCase):
    ...
    def _node_ready_to_submit(self):
        context = dung_boi_canh(self.db)
        service_line_id = context["hang_muc"][0]["id"]
        node_id = them_buoc_k01(self.db, service_line_id=service_line_id, checklist=[])
        user_id = nguoi_dung(self.db)
        employee_id = giao_viec(self.db, task_node_id=node_id, user_id=user_id)
        self.db.execute(
