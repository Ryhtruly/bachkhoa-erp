...
@unittest.skipUnless(os.getenv("TEST_DATABASE_URL") and not os.getenv("TEST_DATABASE_URL", "").startswith("sqlite"), "cần PostgreSQL TEST_DATABASE_URL")
class AuditSqlTrucTiepGiuDauActorTests(unittest.TestCase):
    ...
    def test_khong_con_cau_case_when_nuot_actor(self):
        ...
        for path in (ROOT_SRC / "dossiers/slot_requests.py", ROOT_SRC / "dossiers/register.py"):
