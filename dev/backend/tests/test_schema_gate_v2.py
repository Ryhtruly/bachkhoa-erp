...
class QuyenXinMienTests(unittest.TestCase):
    ...
    def _db(self, *, o_thuoc_hd="HD-1", pham_vi="CONTRACT", o_cua_hm=None,
        ...
        def _tra_loi(cau_lenh, *_args, **_kw):
            sql = str(cau_lenh)
            if "from dossier_document_slots s where s.id" in sql or "from public.dossier_document_slots s where s.id" in sql:
            ...
            if "from service_lines where id" in sql or "from public.service_lines where id" in sql:
                return MagicMock(**{"mappings.return_value.first.return_value": {
            ...
            if "status from" in sql and "document_slot_change_requests" in sql:
            ...
            if "insert into" in sql and "document_slot_change_requests" in sql:
...
class GiaoThucChonGiayTests(unittest.TestCase):
    ...
    def test_duong_tu_dong_khai_DEFAULT_tuong_minh_trong_code(self):
        ...
        for duong in (ROOT_SRC / "routes/routes_crm.py",
...
class HangChoDuyetChiuDuocSchemaCuTests(unittest.TestCase):
    ...
    def test_khong_truy_van_nao_doc_sl_name(self):
        ...
        for duong in (ROOT_SRC / "routes/routes_document_register.py",
                      ROOT_SRC / "dossiers/slot_requests.py",
                      ROOT_SRC / "dossiers/register.py"):
            src = open(duong, encoding="utf-8").read()
