...
def _has_register_version_column(db: Session) -> bool:
    ...
    if "value" not in _SCHEMA_CO_VERSION:
        co = _has_column(db, "service_lines", "document_register_version")
...
def open_service_line_register(
    ...
    line = db.execute(
        text("""
            ...
            from service_lines sl where sl.id = :id
        """),
        {"id": service_line_id},
    ).mappings().first()
    if not line:
...
_WAIVER_PENDING_SQL = """exists (
             select 1 from document_slot_change_requests r
    ...
    *,
    ...
    row = db.execute(
        text("""
            ...
            from dossier_document_slots where id = :id
        """),
        {"id": slot_id},
    ).mappings().first()
    if not row:
...
def slot_unlocked(db: Session, slot_id: str) -> bool:
    return bool(db.execute(
        text("""
            select 1 from document_slot_change_requests
...
def request_slot_change(db: Session, slot_id: str, *, reason: str, requester_id: str) -> dict[str, Any]:
    ...
    request_id = db.execute(
        text("""
            insert into document_slot_change_requests (slot_id, requested_by, reason)
...
def request_slot_waiver(
    ...
    row = db.execute(
        text("""
            ...
            from dossier_document_slots s where s.id = :id
        """),
        {"id": slot_id},
    ).mappings().first()
    if not row:
    ...
    pending_request = db.execute(
        text("""
            ...
            from document_slot_change_requests r
            left join service_lines sl on sl.id = r.service_line_id
    ...
    request_id = db.execute(
        text("""
            insert into document_slot_change_requests
...
def revoke_slot_waiver(
    ...
    row = db.execute(
        text("""
            select r.id, s.name
            from document_slot_change_requests r
            join dossier_document_slots s on s.id = r.slot_id
    ...
    db.execute(
        text("""
            update document_slot_change_requests
...
def link_source_document(
    ...
    *,
    ...
    if existing:
        ...
        link_id = db.execute(
            text("""
                insert into dossier_document_links
...
def unlink_source_document(
    ...
    *,
    ...
    remaining = db.execute(
        text("""
            select
              (select count(*) from dossier_document_links
...
_K01_SLOTS_TMPL = """
            ...
            from dossier_document_slots s
            left join dossier_document_links l on l.slot_id = s.id
            left join dossier_documents d on d.id = l.document_id
__WAIVER_JOIN__
            where s.contract_id = :contract_id
...
def k01_blockers(db: Session, service_line_id: str) -> dict[str, Any]:
    ...
    context = db.execute(
        text("""
            ...
            from service_lines where id = :service_line_id
        """),
        {"service_line_id": service_line_id},
    ).mappings().first()
    if not context:
    ...
    stats = db.execute(
        text("""
            select
              ...
              (select count(*) from dossier_document_links l
               join dossier_documents linked on linked.id = l.document_id
...
def add_slot(
    ...
    *,
    ...
    trung = db.execute(
        text("""
            select 1 from dossier_document_slots
...
def remove_slot(db: Session, slot_id: str, *, actor_id: str) -> dict[str, Any]:
    ...
    db.execute(
        text("delete from dossier_document_slots where id = :id"), {"id": slot_id}
    )
    ...
    rows = db.execute(
        text("""
            select s.id, s.name, s.source, s.scope, s.needs_original, s.quantity,
                   ...
                   (select count(*) from dossier_documents d where d.slot_id = s.id) as file_count
            from dossier_document_slots s
            left join service_lines sl on sl.id = s.service_line_id
            left join task_types tt on tt.id = sl.task_type_id
...
def promote_slot_to_template(db: Session, slot_id: str, *, actor_id: str) -> dict[str, Any]:
    ...
    template_id = db.execute(
        text("""
            insert into document_checklist_templates
...
def list_templates(db: Session) -> dict[str, Any]:
    ...
    task_types = db.execute(
        text("select id, name from task_types order by name")
    ).mappings().all()
    return {
...
def deactivate_template(db: Session, template_id: str) -> dict[str, Any]:
    ...
    row = db.execute(
        text("select id, name from document_checklist_templates where id = :id"),
        {"id": template_id},
    ).mappings().first()
    if not row:
